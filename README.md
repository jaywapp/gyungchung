...
`supabase/migrations`의 마이그레이션은 회원·가입 신청·회비·공지·일정·실제 출석·용병·팀 편성·경기 기록·MOM·의견·투표·설문·선거 테이블, 역할별 권한과 RLS 정책을 생성합니다. 처음 로그인한 사용자는 이름, 전화번호, 생년월일, 거주지역과 선호 포지션을 입력해 가입을 신청해야 하며, 매니저 또는 어드민 승인 전까지 회원 기능을 사용할 수 없습니다.
