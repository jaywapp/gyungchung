...
test("submission RPC still enforces required questions while optional answers may be absent", () => {
  assert.match(migration, /where q\.form_id = target_form_id\s+and q\.is_required/);
  assert.match(migration, /not exists \([\s\S]*?jsonb_array_elements\(submitted_answers\)/);
  assert.match(migration, /Required answer is missing for question %/);
});
