...
test("keeps form controls at the iOS no-zoom font size", () => {
  ...
  assert.match(styles, /\.voice-form input:not\(\[type="checkbox"\]\),[\s\S]*?\.voice-form textarea\s*\{\s*font-size:\s*16px;/);
});
...
test("keeps visible event labels at or above twelve pixels", () => {
  ...
  assert.match(styles, /\.detail-team li span\s*\{[^}]*font-size:\s*12px;/s);
});
