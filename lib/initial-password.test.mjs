...
const triggerFixMigration = readFileSync("supabase/migrations/20260822134406_fix_password_change_trigger_schema.sql", "utf8");
...
test("ends the invalidated browser session and asks the member to sign in again", () => {
  assert.match(passwordPage, /auth\.signOut\(\{ scope: "local" \}\)/);
  assert.match(passwordPage, /window\.location\.replace\("\/\?auth=password-updated"\)/);
  assert.match(clubhouse, /"password-updated": \{ message: "비밀번호를 변경했습니다\. 새 비밀번호로 다시 로그인해 주세요\."/);
  assert.match(clubhouse, /setLoginOpen\(result === "password-updated" \|\| result === "phone-only"\)/);
});

test("lets the service role resolve only its explicitly granted private helpers", () => {
  assert.match(triggerFixMigration, /grant usage on schema private to service_role/);
  assert.doesNotMatch(triggerFixMigration, /grant (?:all|execute) on all functions/i);
});
