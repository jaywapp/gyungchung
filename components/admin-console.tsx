...
import type { AccountRole, Attendance, Event, Fee, Feedback, GuestFee, GuestPlayer, Notice, OfficerPermission, OfficerTitle, ParticipationForm, Profile, RolePermission } from "@/lib/types";
...
const roleLabels: Record<AccountRole, string> = { member: "일반 회원", manager: "관리자", admin: "시스템 관리자" };
...
export default function AdminConsole({ profiles, guestPlayers, attendance, fees, guestFees, notices, events, feedback, forms, rolePermissions, officerPermissions, permissions, supabase, reload, toast }: {
  ...
  const sections = useMemo(() => [
    ...
    (permissions.has("elections.manage") || permissions.has("polls.manage") || permissions.has("surveys.manage")) && ["forms", "참여"], (permissions.has("roles.manage") || permissions.has("officers.manage")) && ["permissions", "권한"],
  ...
  return <section className="content admin-page">
    <div className="page-intro"><span className="eyebrow">OPERATIONS DESK</span><h1>팀 운영 관리</h1><p>어드민은 전체 권한을 관리하고, 회장·부회장·총무는 직책별 운영 업무를 담당합니다.</p></div>
    ...
    {section === "permissions" ? <PermissionMatrix roleRows={rolePermissions} officerRows={officerPermissions} canManageSystemRoles={permissions.has("roles.manage")} supabase={supabase} reload={reload} toast={toast} /> : <div className="admin-list">
      {section === "members" && profiles.map((row) => <AdminRow key={row.id} title={row.membership_application?.name ?? row.name} meta={`${row.membership_application?.preferred_position ?? row.position ?? "포지션 미정"} · ${row.role === "manager" && row.officer_title ? officerTitleLabels[row.officer_title] : roleLabels[row.role]} · ${row.role === "manager" ? "월 15,000원" : row.role === "member" && row.fee_plan === "per_event" ? "참여 시 10,000원" : row.role === "member" ? "월 30,000원" : "회비 면제"} · ${row.status === "pending" ? row.membership_application ? "가입 승인 대기" : "신청서 미작성" : row.status}`} onEdit={() => setEditor({ type: "members", row: row as unknown as Record<string, unknown> })} />)}
    ...
    {editor && <AdminEditor config={editor} profiles={profiles} guestPlayers={guestPlayers} events={events} attendance={attendance} permissions={permissions} supabase={supabase} onClose={() => setEditor(null)} onSaved={() => { setEditor(null); toast("저장했습니다."); reload(); }} />}
  </section>;
}
...
function PermissionMatrix({ roleRows, officerRows, canManageSystemRoles, supabase, reload, toast }: { roleRows: RolePermission[]; officerRows: OfficerPermission[]; canManageSystemRoles: boolean; supabase: SupabaseClient; reload: () => void; toast: (message: string) => void }) {
...
function AdminEditor({ config, profiles, guestPlayers, events, attendance, permissions, supabase, onClose, onSaved }: { config: EditorConfig; profiles: Profile[]; guestPlayers: GuestPlayer[]; events: Event[]; attendance: Attendance[]; permissions: Set<string>; supabase: SupabaseClient; onClose: () => void; onSaved: () => void }) {
  ...
  const submit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (config.type === "members") {
      ...
      ({ error } = await supabase.from("profiles").update({ name: fd.get("name"), position: fd.get("position") || null, jersey_number: Number(fd.get("jersey_number")) || null, role: nextRole, officer_title: canManageRoles ? nextRole === "manager" ? fd.get("officer_title") : null : row.officer_title ?? null, fee_plan: canManageRoles ? nextRole === "member" ? fd.get("fee_plan") : null : row.fee_plan ?? null, status: fd.get("status") }).eq("id", row.id));
    }
  ...
  return <div className="modal-backdrop" onClick={onClose}><form className="editor" onSubmit={submit} onClick={(event) => event.stopPropagation()}><button type="button" className="modal-close" aria-label="닫기" onClick={onClose}><X /></button><span className="eyebrow">ADMIN EDITOR</span><h2>{row.id ? "정보 수정" : "새 항목 등록"}</h2>
    {config.type === "members" && <><label>이름<input name="name" required defaultValue={String(application?.name ?? row.name ?? "")} /></label>{application ? <div className="application-review"><b>가입 신청 정보</b><dl><div><dt>전화번호</dt><dd>{application.phone}</dd></div><div><dt>생년월일</dt><dd>{new Date(`${application.birth_date}T00:00:00`).toLocaleDateString("ko-KR")}</dd></div><div><dt>거주지역</dt><dd>{application.residence}</dd></div><div><dt>선호 포지션</dt><dd>{application.preferred_position}</dd></div><div><dt>신청일</dt><dd>{new Date(application.submitted_at).toLocaleDateString("ko-KR")}</dd></div></dl></div> : row.status === "pending" && <div className="read-box"><b>가입 신청서 미작성</b><p>신청서가 제출되기 전에는 회원 승인을 할 수 없습니다.</p></div>}<div className="field-row"><label>등록 포지션<input name="position" defaultValue={String(application?.preferred_position ?? row.position ?? "")} /></label><label>등번호<input name="jersey_number" type="number" min="0" max="99" defaultValue={String(row.jersey_number ?? "")} /></label></div><div className="field-row"><label>계정 권한<select name="role" value={selectedRole} disabled={!permissions.has("roles.manage")} onChange={(event) => setSelectedRole(event.target.value as AccountRole)}><option value="member" disabled={row.role === "admin"}>일반 회원</option><option value="manager" disabled={row.role === "admin"}>관리자</option><option value="admin" disabled={row.role !== "admin"}>시스템 관리자</option></select></label>{selectedRole === "manager" ? <label>관리자 직책<select name="officer_title" required defaultValue={String(row.officer_title ?? "president")} disabled={!permissions.has("roles.manage")}><option value="president">회장</option><option value="vice_president">부회장</option><option value="treasurer">총무</option></select></label> : selectedRole === "member" ? <label>회비 방식<select name="fee_plan" required defaultValue={String(row.fee_plan ?? "monthly")} disabled={!permissions.has("roles.manage")}><option value="monthly">월회비 · 30,000원</option><option value="per_event">참여 시 · 10,000원</option></select></label> : <div className="read-box"><b>회비 면제</b><p>시스템 관리자 계정에는 회비를 부과하지 않습니다.</p></div>}</div>{selectedRole === "manager" && <p className="form-description">관리자 월회비는 직책과 관계없이 15,000원입니다.</p>}<label>상태<select name="status" defaultValue={String(row.status ?? "pending")}><option value="pending" disabled={row.role === "admin"}>승인 대기</option><option value="active" disabled={!canApprove}>활동</option><option value="inactive" disabled={row.role === "admin"}>비활동</option></select></label></>}
