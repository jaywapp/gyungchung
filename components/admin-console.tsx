...
export default function AdminConsole({ profiles, fees, notices, events, feedback, forms, rolePermissions, permissions, supabase, reload, toast }: {
  ...
  return <section className="content admin-page">
    ...
    {section === "permissions" ? <PermissionMatrix rows={rolePermissions} supabase={supabase} reload={reload} toast={toast} /> : <div className="admin-list">
      {section === "members" && profiles.map((row) => <AdminRow key={row.id} title={row.name} meta={`${row.position ?? "포지션 미정"} · ${roleLabels[row.role]} · ${row.status}`} onEdit={() => setEditor({ type: "members", row: row as unknown as Record<string, unknown> })} />)}
...
function AdminEditor({ config, profiles, permissions, supabase, onClose, onSaved }: { config: EditorConfig; profiles: Profile[]; permissions: Set<string>; supabase: SupabaseClient; onClose: () => void; onSaved: () => void }) {
  ...
  return <div className="modal-backdrop" onClick={onClose}><form className="editor" onSubmit={submit} onClick={(event) => event.stopPropagation()}><button type="button" className="modal-close" aria-label="닫기" onClick={onClose}><X /></button><span className="eyebrow">ADMIN EDITOR</span><h2>{row.id ? "정보 수정" : "새 항목 등록"}</h2>
    {config.type === "members" && <><label>이름<input name="name" required defaultValue={String(row.name ?? "")} /></label><div className="field-row"><label>포지션<input name="position" defaultValue={String(row.position ?? "")} /></label><label>등번호<input name="jersey_number" type="number" min="0" max="99" defaultValue={String(row.jersey_number ?? "")} /></label></div><div className="field-row"><label>직책<select name="role" defaultValue={String(row.role ?? "member")}><option value="member">회원</option><option value="president">회장</option><option value="vice_president">부회장</option><option value="treasurer">총무</option></select></label><label>상태<select name="status" defaultValue={String(row.status ?? "pending")}><option value="pending">승인 대기</option><option value="active">활동</option><option value="inactive">비활동</option></select></label></div></>}
