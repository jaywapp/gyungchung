...
import { getEventCapacity, type EventCapacity } from "@/lib/event-capacity";
...
export default function Clubhouse({ children }: { children?: React.ReactNode }) {
  ...
  const setMyAttendance = async (status: Attendance["status"], eventId = upcoming?.id) => {
    ...
    if (status === "going" && existing?.status !== "going" && event) {
      ...
      if (capacity.status === "full" || capacity.status === "over_capacity") {
        const detail = capacity.status === "over_capacity" ? `이미 정원을 ${Math.abs(capacity.remaining ?? 0)}명 초과했습니다.` : "현재 정원이 모두 찼습니다.";
        if (!window.confirm(`${detail} 참석으로 등록하면 정원을 초과하게 됩니다. 계속할까요?`)) return;
      }
    }
...
function Home({ upcoming, notice, feeStanding, goingCount, memberCount, user, restriction, publicLoading, eventLoadError, noticeLoadError, onRetry, onNavigate, onAttendance, myAttendance }: { upcoming?: Event; notice?: Notice; feeStanding: FeeStanding | null; goingCount: number; memberCount: number; user: User | null; restriction: ReturnType<typeof getMembershipRestriction>; publicLoading: boolean; eventLoadError: boolean; noticeLoadError: boolean; onRetry: () => void; onNavigate: (tab: Tab) => void; onAttendance: (status: Attendance["status"]) => void; myAttendance?: Attendance["status"] }) {
  ...
  return <><section className="hero"><div className="hero-copy"><span className="eyebrow"><span /> EST. 2014 · SEOUL</span><h1>우리의 주말,<br /><em>우리의 풋살.</em></h1><div className="lead-row"><strong>2026<br />SEASON</strong><p>경쟁보다 함께 뛰는 즐거움. 경충FC는 주말마다 모여 공을 차고, 땀 흘리고, 오래 함께할 사람을 만듭니다.</p></div><div className="hero-actions"><button className="cta" onClick={() => onNavigate("events")}>다음 일정 참석하기 <ChevronRight /></button><button className="text-link" onClick={() => onNavigate("participation")}>팀 의사결정 참여</button></div></div>
    ...
    <section className="locker"><div className="section-heading"><div><span className="eyebrow">MEMBER LOCKER ROOM</span><h2>팀 클럽하우스</h2></div><span suppressHydrationWarning>{now.getFullYear()} · {String(now.getMonth() + 1).padStart(2, "0")}</span></div><div className="panel-grid">
      <article className="panel"><div className="panel-title"><CalendarDays /><span><small>NEXT SCHEDULE RSVP</small><h3>참석 여부</h3></span></div>{upcoming ? <div className="rsvp-schedule"><time dateTime={upcoming.starts_at}>{formatDate(upcoming.starts_at)} · {formatTime(upcoming.starts_at)}</time><a href={naverMapUrl(upcoming)} target="_blank" rel="noreferrer"><MapPin size={16} /><span><b>{upcoming.venue}</b><small>{upcoming.address || "네이버 지도에서 위치 확인"}</small></span><ChevronRight size={16} /></a></div> : <p>아직 등록된 다음 일정이 없습니다.</p>}{capacity && <CapacityStatus capacity={capacity} />}{upcoming && <><div className="rsvp"><button className={myAttendance === "going" ? "selected" : ""} aria-pressed={myAttendance === "going"} disabled={Boolean(restriction)} aria-describedby={restriction ? "home-rsvp-restriction" : undefined} onClick={() => onAttendance("going")}>{user ? "참석" : "로그인 후 참석"}</button><button className={myAttendance === "not_going" ? "selected no" : ""} aria-pressed={myAttendance === "not_going"} disabled={Boolean(restriction)} aria-describedby={restriction ? "home-rsvp-restriction" : undefined} onClick={() => onAttendance("not_going")}>불참</button></div>{restriction && <p className="restriction-reason" id="home-rsvp-restriction">{getMembershipRestrictionCopy(restriction).action}</p>}</>}</article>
...
function CapacityStatus({ capacity }: { capacity: EventCapacity }) {
  const detail = capacity.status === "unlimited"
  ...
  return <span className={`capacity-status ${capacity.status}`}><b>{capacity.capacity === null ? "정원 제한 없음" : `참석 ${capacity.totalCount} / 정원 ${capacity.capacity}명`}</b><small>{detail}</small></span>;
}
