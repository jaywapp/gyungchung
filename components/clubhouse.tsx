...
function UnlinkedAccountModal({ onClose, onSignOut }: { onClose: () => void; onSignOut: () => Promise<void> }) {
  const dialogRef = useDialogFocus<HTMLDivElement>({ onRequestClose: onClose });
  return <div className="modal-backdrop" onClick={onClose}><div ref={dialogRef} tabIndex={-1} className="login-modal" onClick={(event) => event.stopPropagation()} role="dialog" aria-modal="true" aria-label="계정 연결 필요"><button type="button" className="modal-close" onClick={onClose} aria-label="닫기"><X /></button><span className="eyebrow">ACCOUNT NOTICE</span><h2>계정 연결이 필요합니다</h2><div className="read-box"><b>회원 프로필을 찾지 못했습니다</b><p>로그인은 완료됐지만 이 계정은 아직 경충FC 회원 프로필에 연결되지 않았습니다.</p></div><p className="form-description">경충FC는 운영진이 계정을 등록합니다. 가입 요청 또는 계정 연결이 필요하면 운영진에게 이름과 전화번호를 알려 주세요. 다른 계정으로 로그인하려면 먼저 로그아웃할 수 있습니다.</p><Link href={tabPaths.feedback} className="cta secondary" onClick={onClose}>운영진에게 문의하기 <ChevronRight size={17} /></Link><button type="button" className="cta secondary" onClick={() => void onSignOut()}><LogOut size={17} /> 로그아웃</button></div></div>;
}

function Home({ upcoming, notice, feeStanding, goingCount, memberCount, user, profile, sessionPending, rsvpPending, publicLoading, eventLoadError, noticeLoadError, onRetry, onNavigate, onAttendance, onLogin, myAttendance }: { upcoming?: Event; notice?: Notice; feeStanding: FeeStanding | null; goingCount: number; memberCount: number; user: User | null; profile: Profile | null; sessionPending: boolean; rsvpPending: boolean; publicLoading: boolean; eventLoadError: boolean; noticeLoadError: boolean; onRetry: () => void; onNavigate: (tab: Tab) => void; onAttendance: (status: Attendance["status"]) => void; onLogin: () => void; myAttendance?: Attendance["status"] }) {
