...
export default function Clubhouse() {
  ...
  const kickMember = async (profile: Profile) => {
    if (!supabase || profile.is_system_admin || !window.confirm(`${profile.name} 회원을 강퇴할까요? 다시 가입하려면 운영진이 상태를 변경해야 합니다.`)) return;
    const { error } = await supabase.from("profiles").update({ status: "inactive" }).eq("id", profile.id);
    if (error) return showToast(toErrorMessage(error), "error");
    showToast(`${profile.name} 회원을 강퇴했습니다.`);
    await loadData(user);
  };
...
function Members({ profiles, user, canManage, onEdit, onKick, onLogin }: { profiles: Profile[]; user: User | null; canManage: boolean; onEdit: (profile: Profile) => void; onKick: (profile: Profile) => void; onLogin: () => void }) { if (!user) return <section className="content"><PageIntro kicker="SQUAD" title="함께 뛰는 사람들" description="회원 명단은 승인된 회원에게만 공개합니다." /><button className="cta" onClick={onLogin}>로그인하고 확인하기 <ChevronRight /></button></section>; return <section className="content"><PageIntro kicker="SQUAD" title="함께 뛰는 사람들" description="포지션보다 이름을 먼저 기억하는 경충FC의 회원입니다." />{canManage && <div className="inline-management-note"><Shield size={17} /> 회원 정보를 수정하거나 활동 회원을 강퇴할 수 있습니다.</div>}<div className="member-grid">{profiles.map((profile, index) => <article className="member-card" key={profile.id}><span className="member-number">{profile.jersey_number ?? String(index + 1).padStart(2, "0")}</span>{canManage && <div className="member-management-actions"><button aria-label={`${profile.name} 회원 정보 수정`} onClick={() => onEdit(profile)}><Pencil size={16} /></button>{profile.id !== user.id && !profile.is_system_admin && <button aria-label={`${profile.name} 회원 강퇴`} onClick={() => onKick(profile)}><UserMinus size={16} /></button>}</div>}<div className="avatar"><UserRound /></div><small>{profile.position ?? "PLAYER"}</small><h2>{profile.name}</h2><p>JOINED {new Date(profile.joined_at).getFullYear()}</p><div className="member-badges">{profile.role === "manager" && <span className="admin-badge">{profile.officer_title ? officerTitleLabels[profile.officer_title] : roleLabels.manager}</span>}{profile.is_system_admin && <span className="admin-badge system">시스템 관리자</span>}</div></article>)}</div></section>; }
...
function Rankings({ rankings, momLeaderboard, user, profile, onLogin }: { rankings: MemberRanking[]; momLeaderboard: MomLeaderboardEntry[]; user: User | null; profile: Profile | null; onLogin: () => void }) {
  ...
  return <section className="content"><PageIntro kicker="CLUB RANKING" title="활동 랭킹" description="실제 출석 1회는 3점, 회비 납부 1개월은 1점으로 집계합니다. 금액과 개인 연락처는 공개하지 않습니다." /><div className="ranking-layout"><div><div className="section-heading compact"><div><span className="eyebrow">ACTIVITY</span><h2>활동 순위</h2></div></div><div className="table-wrap ranking-table"><table><caption className="sr-only">회원별 출석 및 회비 납부 활동 순위</caption><thead><tr><th scope="col">순위</th><th scope="col">회원</th><th scope="col">실제 출석</th><th scope="col">회비 납부</th><th scope="col">종합 점수</th></tr></thead><tbody>{rankings.map((ranking, index) => <tr key={ranking.member_id}><td><span className="rank-badge">{index + 1}</span></td><th scope="row">{ranking.member_name}</th><td data-label="실제 출석">{ranking.attendance_count}회</td><td data-label="회비 납부">{ranking.paid_fee_count}개월</td><td data-label="종합 점수"><b>{ranking.total_score}점</b></td></tr>)}</tbody></table>{rankings.length === 0 && <Empty icon={<CalendarDays />} title="아직 집계된 기록이 없습니다" description="실제 출석이나 회비 납부 기록이 등록되면 순위가 표시됩니다." />}</div></div><div><div className="section-heading compact"><div><span className="eyebrow">MOM HALL OF FAME</span><h2>MOM 명예의 전당</h2></div></div><div className="table-wrap ranking-table"><table><caption className="sr-only">회원별 MOM 수상 및 득표 순위</caption><thead><tr><th scope="col">순위</th><th scope="col">회원</th><th scope="col">1위</th><th scope="col">2위</th><th scope="col">3위</th><th scope="col">득표</th></tr></thead><tbody>{momLeaderboard.map((ranking, index) => <tr key={ranking.member_id}><td><span className="rank-badge">{index + 1}</span></td><th scope="row">{ranking.member_name}</th><td data-label="1위">{ranking.first_place_count}회</td><td data-label="2위">{ranking.second_place_count}회</td><td data-label="3위">{ranking.third_place_count}회</td><td data-label="득표">{ranking.total_votes}표</td></tr>)}</tbody></table></div></div></div></section>;
}
