...
export default function Clubhouse({ children }: { children?: React.ReactNode }) {
  ...
  return <div className="page">
    ...
    <main id="main">
      ...
      {tab === "rankings" && <Rankings rankings={rankings} goalEvents={events} momLeaderboard={momLeaderboard} user={user} profile={me} loading={sessionPending || publicLoading} loadError={memberLoadError || eventLoadError} onLogin={() => setLoginOpen(true)} onRetry={() => void reload()} />}
...
function Events({ events, profiles, attendance, momVotes, momResults, user, profile, supabase, loading, loadError, canManage, onCreate, onEdit, onManageMatch, onManageAttendance, onDelete, onAttendance, onLogin, onRetry, reload, toast }: { events: Event[]; profiles: Profile[]; attendance: Attendance[]; momVotes: EventMomVote[]; momResults: EventMomResult[]; user: User | null; profile: Profile | null; supabase: ReturnType<typeof createClient>; loading: boolean; loadError: boolean; canManage: boolean; onCreate: () => void; onEdit: (event: Event) => void; onManageMatch: (event: Event) => void; onManageAttendance: (event: Event) => void; onDelete: (id: string, label: string) => void; onAttendance: (status: Attendance["status"], eventId?: string) => void; onLogin: () => void; onRetry: () => void; reload: () => void; toast: ToastHandler }) {
  ...
  return <section className="content">{intro}{canManage && <div className="page-management-actions"><button className="cta small" onClick={onCreate}><Plus size={17} /> 일정 등록</button></div>}<MonthCalendar events={events} />{events.length === 0 ? <Empty icon={<CalendarDays />} title="등록된 일정이 없습니다" description="운영진이 주말 일정을 등록하면 이곳에 표시됩니다." /> : <div className="event-list">{events.map((event, index) => {
    ...
    const canVote = Boolean(user && profile?.status === "active" && isPast && attendance.some((row) => row.event_id === event.id && row.member_id === profile.id && isCheckedIn(row)));
    ...
    const lateCount = eventAttendance.filter((item) => getCheckInStatus(item) === "late").length;
    const absentCount = eventAttendance.filter((item) => getCheckInStatus(item) === "absent").length;
    return <article id={`event-${event.id}`} key={event.id} className={`event-card${isPast ? " past" : ""}`}><time className="event-date" dateTime={event.starts_at}><small>{eventDate.getFullYear()} · {eventDate.toLocaleDateString("ko-KR", { month: "long" })}</small><b>{String(eventDate.getDate()).padStart(2, "0")}</b><span>{eventDate.toLocaleDateString("ko-KR", { weekday: "long" })}</span></time><div className="event-info"><small>{isPast ? "지난 일정" : "WEEKLY FUTSAL"} · #{String(index + 1).padStart(2, "0")}{event.is_competitive ? " · 커피 내기" : ""}</small><h2>{event.title}</h2><p><MapPin size={16} /> <a className="inline-map-link" href={naverMapUrl(event)} target="_blank" rel="noreferrer">{event.venue}</a><span className="event-meta-time">· {formatTime(event.starts_at)}</span></p><span>{event.note}</span>{guests.length > 0 && <div className="guest-roster"><b>참여 용병</b>{guests.map((guest) => <span key={guest.guest_player_id}>{guest.guest_name}{guest.guest_position ? ` · ${guest.guest_position}` : ""}</span>)}</div>}{teams.length > 0 && <div className="event-teams">{teams.map((team) => { const result = team.score === null || maxScore === null ? null : team.score === maxScore ? winnerCount > 1 ? "무승부" : "승리" : "패배"; return <div className="event-team" key={team.id}><div><b>{team.team_name}</b>{event.is_competitive && <span>{team.score ?? "-"}점{result ? ` · ${result}` : ""}</span>}</div><p className="event-team-roster">{team.event_team_members.map((member) => <span className="event-team-player" key={member.id}><span>{member.participant_name}</span>{member.goals > 0 && <span className="event-team-player-goals" role="img" aria-label={String(member.goals) + "골"}>{"⚽️".repeat(member.goals)}</span>}{member.rating !== null && <span className="event-team-player-rating">{member.rating}점</span>}</span>)}</p></div>; })}</div>}{matches.length > 0 && <div className="event-match-history"><div className="event-match-heading"><b>경기 결과</b><span>{matches.length}경기</span></div>{matches.map((match) => { const teamAName = teamNameById.get(match.team_a_id) ?? "팀 A"; const teamBName = teamNameById.get(match.team_b_id) ?? "팀 B"; const scorersFor = (teamId: string) => match.event_match_scorers.filter((scorer) => scorer.team_id === teamId);
      const renderScorers = (teamId: string, teamName: string) => {
        const scorers = scorersFor(teamId);
        if (scorers.length === 0) return null;
        return <span className="event-match-scorer-group"><b>{teamName}</b><span className="event-match-scorer-list">{scorers.map((scorer) => <span className="event-match-scorer" key={scorer.id}><span>{scorer.scorer_name}</span><span className="event-match-balls" role="img" aria-label={String(scorer.goals) + "골"}>{"⚽️".repeat(scorer.goals)}</span></span>)}</span></span>;
      };
      const teamAScorers = scorersFor(match.team_a_id);
      const teamBScorers = scorersFor(match.team_b_id);
      return <div className="event-match" key={match.id}><div><b>{match.match_number}경기</b><span>{teamAName} <strong>{match.team_a_score}:{match.team_b_score}</strong> {teamBName}</span></div>{(teamAScorers.length > 0 || teamBScorers.length > 0) && <p className="event-match-scorers">{renderScorers(match.team_a_id, teamAName)}{teamAScorers.length > 0 && teamBScorers.length > 0 && <span className="event-match-scorer-separator" aria-hidden="true">·</span>}{renderScorers(match.team_b_id, teamBName)}</p>}</div>;})}</div>}{eventResults.length > 0 && <div className="mom-results"><b>MOM TOP 3</b>{eventResults.map((result) => <span key={result.candidate_profile_id}>{result.mom_rank}위 {result.candidate_name} · {result.vote_count}표</span>)}</div>}</div><div className="event-action"><b>참석 예정 {count}명 · 출석 {presentCount}명 · 지각 {lateCount}명 · 결석 {absentCount}명 · 용병 {guests.length}명</b>{canManage && <div className="resource-actions"><button aria-label={`${event.title} 수정`} onClick={() => onEdit(event)}><Pencil size={16} /></button><button aria-label={`${event.title} 삭제`} onClick={() => onDelete(event.id, `${formatDate(event.starts_at)} · ${event.title}`)}><Trash2 size={16} /></button></div>}<button className="cta small" disabled={isPast} onClick={() => user ? onAttendance("going", event.id) : onLogin()}>{isPast ? "지난 일정" : "참석하기"}</button>{canManage && <><button className="text-link" onClick={() => onManageAttendance(event)}>출석 체크</button><button className="text-link" onClick={() => onManageMatch(event)}>팀·경기 기록</button></>}{canVote && <button className="text-link" onClick={() => setVotingEvent(event)}>{ownVote ? "MOM 재투표" : "MOM 투표"}</button>}</div></article>;
  })}</div>}{votingEvent && <div className="modal-backdrop" onClick={() => setVotingEvent(null)}><div ref={momDialogRef} tabIndex={-1} className="editor mom-vote-modal" role="dialog" aria-modal="true" aria-label="MOM 투표" onClick={(event) => event.stopPropagation()}><button className="modal-close" aria-label="닫기" onClick={() => setVotingEvent(null)}><X /></button><span className="eyebrow">MAN OF THE MATCH</span><h2>MOM 투표</h2><p className="form-description">MOM(Man of the Match)은 그날 경기의 최우수 선수입니다. 실제 출석한 회원 중 한 명을 선택해 주세요. 본인에게는 투표할 수 없고, 마감 전까지 다시 고를 수 있습니다.</p><div className="mom-candidates">{profiles.filter((candidate) => candidate.id !== user?.id && candidate.status === "active" && attendance.some((row) => row.event_id === votingEvent.id && row.member_id === candidate.id && isCheckedIn(row))).map((candidate) => { const selected = momVotes.find((vote) => vote.event_id === votingEvent.id)?.candidate_profile_id === candidate.id; return <button key={candidate.id} className={selected ? "selected" : ""} aria-pressed={selected} onClick={() => void submitMomVote(candidate.id)}><span>{candidate.position ?? "PLAYER"}</span><b>{candidate.name}</b></button>; })}</div></div></div>}</section>;
}
