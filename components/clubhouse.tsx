...
export default function Clubhouse() {
  ...
  return <main className="page">
    ...
    <div className="toast" role="status" aria-live="polite" aria-atomic="true">{toast?.kind === "success" && <><Check size={17} />{toast.message}</>}</div>
    <div className="toast error" role="alert" aria-live="assertive" aria-atomic="true">{toast?.kind === "error" && <><AlertCircle size={17} />{toast.message}</>}</div>
  </main>;
}
...
function Rankings({ rankings, momLeaderboard, user, profile, onLogin }: { rankings: MemberRanking[]; momLeaderboard: MomLeaderboardEntry[]; user: User | null; profile: Profile | null; onLogin: () => void }) {
  if (!user) return <section className="content"><PageIntro kicker="CLUB RANKING" title="활동 랭킹" description="실제 출석과 회비 납부 기록으로 보는 경충FC 활동 대시보드입니다." /><LoginGate icon={<Shield />} title="로그인 후 활동 기록을 확인하세요" description="출석과 회비 납부 기록으로 집계한 회원 전용 대시보드입니다." onLogin={onLogin} /></section>;
...
function LoginGate({ icon, title, description, onLogin }: { icon: React.ReactNode; title: string; description: string; onLogin: () => void }) { return <div className="login-gate"><Empty icon={icon} title={title} description={description} /><button className="cta" onClick={onLogin}>로그인하고 확인하기 <ChevronRight /></button></div>; }
